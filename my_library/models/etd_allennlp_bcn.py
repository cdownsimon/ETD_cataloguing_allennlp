...
@Model.register("etd_allennlp_bcn")
class BiattentiveClassificationNetwork(Model):
    ...
    def forward(self,  # type: ignore
        ...
        Parameters
        ----------
        abstract_text : Dict[str, torch.LongTensor], required
        ...
        """
        text_mask = util.get_text_field_mask(abstract_text).float()
        # Pop elmo tokens, since elmo embedder should not be present.
        elmo_tokens = abstract_text.pop("elmo", None)
        if abstract_text:
            embedded_text = self._text_field_embedder(abstract_text)
        else:
        ...
        if elmo_tokens is not None:
            abstract_text["elmo"] = elmo_tokens
